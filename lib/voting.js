...
module.exports = function(config, gh, Twitter) {
  ...
  function postVoteStarted(pr) {
    getVoteStartedComment(pr, function(err, comment) {
      ...
      gh.issues.createComment({
        ...
        // Tweet vote started
        Twitter.postTweet('Vote started for PR #' + pr.number + ': https://github.com/canterbot/canterbot/pull/' + pr.number);
  ...
  function closePR(message, pr, cb) {
    ...
    gh.pullRequests.update({
      ...
      // Tweet PR closed
      Twitter.postTweet('PR #' + pr.number + ' has been closed: https://github.com/canterbot/canterbot/pull/' + pr.number);
      ...
      gh.pullRequests.merge({
        ...
        if(!err){ 
          ...
          Twitter.postTweet('I now have the ability to ' + pr.title +
            ' https://github.com/canterbot/canterbot/pull/' + pr.number);
        }
        cb(err, res);
      });
    });
  ...
  events.on('github.pull_request.opened', function (data) {
    console.log('EVENT: github.pull_request.opened');
  ...
  events.on('github.pull_request.closed', function (data) {
    console.log('EVENT: github.pull_request.closed');
  ...
  events.on('github.comment.created', function (data) {
    console.log('EVENT: github.comment.created');
